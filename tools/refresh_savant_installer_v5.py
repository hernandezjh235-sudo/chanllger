#!/usr/bin/env python3
"""Challenger Savant refresh v5.

Uses 7-day Statcast detail windows for batter-vs-pitcher-hand reconstruction.
Savant caps detail CSV responses at 25,000 rows; the earlier 14-day windows hit
that ceiling and undercounted PA.  Seven-day windows stay below the cap while
retaining all v3/v4 integrity checks.
"""
...
def _event_split_totals_seven_day() -> pd.DataFrame:
    ...
    while chunk_start <= today:
        chunk_end = min(today, chunk_start + timedelta(days=6))
...
v3._event_split_totals = _event_split_totals_seven_day
